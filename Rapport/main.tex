...
\end{mydef}
An interesting way to see the fiber of a map is via a pullback square \ref{fig:pullbacksquare}. For $b :B$ let $b_*$ be the map $\mathbf{1} \rightarrow B$ sending the canonical element $*$ to $b$. The the pullback of $f$ and $b_*$ is equivalent to the $\mathrm{fib}_f(b)$ : 
\begin{center}
  \begin{tikzcd}
...
\begin{align}
A \times_B \textbf{1} :&= \sum_{x :A} \sum_{y : \textbf{1}} f(x)=b_*(y)\\
&\simeq \sum_{x : A} f(x)=b_*(*) \text{\quad \quad (this follows from the type \textbf{1} being uniquely inhabited, see \cite{hott})}\\
:&=\mathrm{fib}_f(b)
\end{align}
Let $m$ be a nonnegative integer and $r \in \big (\mathbb Z / m\mathbb Z\big )^{\times}$. Then we have a short exact sequence of groups : 
$$\mathbb{Z} \xrightarrow{\times m} \mathbb{Z} \xrightarrow{1 \mapsto \overline r} \mathbb Z / m\mathbb Z$$
Which yields another short exact sequence : 
