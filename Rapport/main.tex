...
\subsection{Equality and Homotopy}
We want to establish the following correspondence in between properties on indentity types and paths in homotopy:
\begin{center}
\begin{tabular}{|c|c|}
...
\end{center}
\begin{prop}
For $A : \mathcal{U}$ and $x,y : A$ there is a function : 
  $$\mathrm{inv} : (x=y) \rightarrow (y=x)$$
such that $\mathrm{inv}(\mathrm{refl}_x) \equiv \mathrm{refl}_x$. We will denote $\mathrm{inv}(p)$ by $p^{-1}$.
\end{prop}
\begin{proof}
By path induction, we only need to define it in the case where $x \equiv y$ and $p \equiv \mathrm{refl}_x$, we then have $(y=x) \equiv (x=x)$ and we terminate the proof by setting $\mathrm{inv}(\mathrm{refl}_x) :\equiv \mathrm{refl}_x$.
\end{proof}
