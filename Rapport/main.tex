...
\section{}
We will be using another version of the flattening lemma with pushout types this time: 
\begin{prop}
  Suppose given a fibration over a pushout type: 
  \begin{center}
    \begin{tikzcd}
      C \arrow[r, "g"] \arrow[d, "f"']                                                         & B \arrow[d, "\mathrm{inr}"] \arrow[ld, "\mathrm{glue}" description, Rightarrow] \arrow[rdd, "P \circ \mathrm{inr}" description, bend left] &            \\
      A \arrow[r, "\mathrm{inl}"'] \arrow[rrd, "P \circ \mathrm{inl}" description, bend right] & A \sqcup_C B \arrow[rd, "P"]                                                                                                               &            \\
                                                                                               &                                                                                                                                            & \mathcal U
      \end{tikzcd}
  \end{center}
  Then, one has a pushout diagram of total spaces :
  \begin{center} 
    \begin{tikzcd}
      \displaystyle \sum_{c : C} P \circ \mathrm{inl} \circ f (c) \arrow[dd, "{(c,x)\mapsto(f(c),x)}"'] \arrow[rr, "{(c,x) \mapsto (g(x),p_c^*(x))}", shift left=2] &  & \displaystyle \sum_{b : B} P \circ \mathrm{inr} (b) \arrow[dd] \arrow[lldd, "\mathrm{glue}" description, Rightarrow] \\
                                                                                                                                                                    &  &                                                                                                                                                           \\
      \sum_{a : A} P \circ \mathrm{inl} (a) \arrow[rr]                                                                       &  & \displaystyle \sum_{x : A \sqcup_C B} P(x)                                                                                                               
    \end{tikzcd}
  \end{center} 
\end{prop}
\printbibliography
