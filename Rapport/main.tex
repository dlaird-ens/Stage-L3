...
\begin{prop}[Flattening Lemma]
  ...
  \begin{center}
    \begin{tikzcd}
      \sum_{x : B} P \circ c \circ f (x) \arrow[rrr, "{(b,x) \mapsto(g(b),p_b^*(x))}", shift left=3] \arrow[rrr, "{(b,x) \mapsto (f(b),x)}"', shift right=3] &  &  & \sum_{x : A} P \circ c (x) \arrow[rr, "{(a,x) \mapsto(c(a),x)}"] &  & \sum_{x : W} P(x)
    \end{tikzcd}
  \end{center}
\end{prop}
